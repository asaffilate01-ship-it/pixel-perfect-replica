    ...
    <footer className="mt-20 bg-navy text-navy-foreground">
      <div className="mx-auto max-w-[1240px] px-5 py-14 lg:px-7 lg:py-18">
        <div className="grid gap-12 lg:grid-cols-[1.35fr_.65fr_.65fr_.65fr]">
          <div>
            <div className="inline-flex rounded-2xl bg-background px-4 py-2"><Logo className="[&_img]:h-10 md:[&_img]:h-10" /></div>
...
function FooterGroup({ title, links }: { title: string; links: readonly (readonly [string, string])[] }) {
  return <div className="text-sm"><h2 className="eyebrow text-leaf-soft">{title}</h2><ul className="mt-5 grid gap-3 text-navy-foreground/70">{links.map(([label, to]) => <li key={label}><a href={to} className="transition-colors hover:text-navy-foreground">{label}</a></li>)}</ul></div>;
}
