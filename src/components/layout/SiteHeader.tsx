    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[82px] max-w-[1240px] items-center justify-between gap-6 px-5 lg:px-7">
        ...
        <nav className="hidden items-center gap-1 text-sm font-semibold text-foreground/75 lg:flex">
          <a href="/#how-it-works" className="rounded-full px-4 py-2 transition-colors hover:bg-secondary hover:text-primary">
            How it works
          </a>
          <div className="relative" onMouseEnter={() => setPlatformOpen(true)} onMouseLeave={() => setPlatformOpen(false)}>
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full px-4 py-2 transition-colors hover:bg-secondary hover:text-primary"
                ...
                <div className="grid grid-cols-2 gap-2 rounded-3xl border border-border bg-card p-3 shadow-lift">
                  {PLATFORM_LINKS.map((item) => (
                    <Link
                      key={item.to}
                      to={item.to}
                      className="rounded-2xl p-4 transition-colors hover:bg-secondary"
                    >
          ...
          <a href="/#who-we-help" className="rounded-full px-4 py-2 transition-colors hover:bg-secondary hover:text-primary">
            Who we help
          </a>
          <a href="/#reva" className="rounded-full px-4 py-2 transition-colors hover:bg-secondary hover:text-primary">
            Reva AI
          </a>
          <a href="/#impact" className="rounded-full px-4 py-2 transition-colors hover:bg-secondary hover:text-primary">
            Impact
          </a>
        </nav>
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          <Button asChild size="sm" className="rounded-full bg-amber px-5 text-amber-foreground hover:bg-amber/90">
      ...
      {open ? (
        <div className="border-t border-border bg-background px-5 py-5 lg:hidden">
          <nav className="mx-auto grid max-w-[1240px] gap-1 text-sm font-semibold">
            ...
            {PLATFORM_LINKS.map((item) => (
              <Link key={item.to} to={item.to} onClick={() => setOpen(false)} className="rounded-xl px-3 py-3 hover:bg-secondary">
                {item.label}
              </Link>
            ))}
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <Button asChild className="bg-amber text-amber-foreground"><Link to="/properties/new">Check property</Link></Button>
            </div>
          </nav>
        </div>
      ) : null}
