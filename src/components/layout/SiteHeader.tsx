...
const AUDIENCE_LINKS = [
  ...
  { to: "/provider/offers", label: "Housing providers" },
      ...
      <div className="mx-auto flex h-[82px] max-w-[1240px] items-center justify-between gap-6 px-5 lg:px-7">
        ...
        <nav className="hidden items-center gap-1 text-sm font-semibold text-foreground/75 lg:flex">
          ...
          <div className="relative" onMouseEnter={() => setPlatformOpen(true)} onMouseLeave={() => setPlatformOpen(false)}>
            <button type="button" onClick={() => setPlatformOpen((v) => !v)} className="inline-flex items-center gap-1 rounded-full px-4 py-2 transition-colors hover:bg-secondary hover:text-primary" aria-expanded={platformOpen}>
        ...
        <button type="button" onClick={() => setOpen((v) => !v)} className="grid size-11 place-items-center rounded-full border border-border bg-card lg:hidden" aria-label="Toggle navigation" aria-expanded={open}>
      ...
      {open ? (
        <div className="border-t border-border bg-background px-5 py-5 lg:hidden">
          <nav className="mx-auto grid max-w-[1240px] gap-1 text-sm font-semibold">
            ...
            {PLATFORM_LINKS.map((item) => (
              <Link key={item.to} to={item.to} onClick={() => setOpen(false)} className="rounded-xl px-3 py-3 hover:bg-secondary">{item.label}</Link>
            ))}
